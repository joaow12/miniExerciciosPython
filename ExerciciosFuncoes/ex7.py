...
def valorPagamento(valorPrestacao,diasAtraso):
    ...
    elif diasAtraso>0 and diasAtraso<=1:
        valorPrestacao += valorPrestacao*multa
        return valorPrestacao
    else:
        multa += diasAtraso*0.001
        valorPrestacao += valorPrestacao*multa
        return valorPrestacao
...
while z != 0:
    z = float(input("Digite o valor da prestação: "))
    if z == 0:
        print("Programa Encerrado")
    else:
        x = int(input("Digite os dias de atraso: "))
        y = valorPagamento(z,x)
        print("O total da prestação é: R$", y)
